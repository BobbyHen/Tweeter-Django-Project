...
# def home(request):
#     return HttpResponse('<h1>Welcome to Tweeter</h1>')
...
#         return Tweet.objects.filter(author = self.request.user).order_by('-date_posted')
#         # return Tweet.objects.filter(author_id=user).order_by('-date_posted')

# @login_required
# def dashboard(request):
#     return render(request, 'users/dashboard.html', {'title': 'dashboard', 'tweets': Tweets.objects.all().order_by('-date_posted')})

# @login_required
# def compose(request):
#     author = ''
#     return render(request, 'users/compose.html', {'title': 'Compose Tweet'})
